...
main <- function() {
    ...
    local_file <- file.path(outdir, sprintf("table_%s_local_%s.csv", dataset, mode))
    bioc_file <- file.path(outdir, sprintf("table_%s_bioc_%s.csv", dataset, mode))
    if (!file.exists(local_file) || !file.exists(bioc_file)) {
        stop("Missing input tables. Run run_dataset_version.R for local and bioc first.")
    }
    ...
    jaccard <- if (uni == 0L) NA_real_ else inter / uni
    ...
    utils::write.csv(metric_df,
        file = file.path(outdir, sprintf("metrics_%s_%s.csv", dataset, mode)),
        row.names = FALSE
    )
    utils::write.csv(overview,
        file = file.path(outdir, sprintf("overview_%s_%s.csv", dataset, mode)),
        row.names = FALSE
    )

    message("Comparison finished.")
    message(sprintf("Overview: %s", file.path(outdir, sprintf("overview_%s_%s.csv", dataset, mode))))
    message(sprintf("Metrics: %s", file.path(outdir, sprintf("metrics_%s_%s.csv", dataset, mode))))
}
